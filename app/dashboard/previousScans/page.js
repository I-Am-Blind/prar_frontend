...
export default function Page() {
  ...
  function extractUniqueDates(readings) {
    ...
    Object.keys(readings).forEach((sensor) => {
      ...
      readings[sensor].forEach((reading) => {
        if (reading.readings === undefined || reading.readings === 'undefined') {
          return;
        }
    ...
    console.log(newStructuredReadings)
  }
  ...
  function renderTimeButtons() {
    ...
    Object.keys(filteredReadings).forEach((sensorKey) => {
      ...
      if (dateReadings) {
        Object.entries(dateReadings).forEach(([time, readings]) => {
  ...
  function renderReadingCards() {
    ...
    Object.keys(filteredReadings).forEach((sensorKey) => {
      const dateReadings = filteredReadings[sensorKey][selectedDate];
    if (dateReadings && dateReadings[selectedTime] && dateReadings[selectedTime].length > 0) {
      const sensorReadings = dateReadings[selectedTime];
        ...
        sensorMap[sensorKey] = sensorReadings.reduce((acc, reading) => {
          ...
          // Store only the latest reading for each timestamp
